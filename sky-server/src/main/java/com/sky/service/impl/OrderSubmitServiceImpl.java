...
public class OrderSubmitServiceImpl implements OrderSubmitService {
    ...
    public void cancel(Long id) throws Exception {
        ...
        if (ordersDB.getStatus().equals(OrderVO.TO_BE_CONFIRMED)) {
            //调用微信支付退款接口
            weChatPayUtil.refund(ordersDB.getNumber(),//订单号
...
//    }
    ...
    public void rejection(OrdersRejectionDTO ordersRejectionDTO) throws Exception {
        ...
        if (payStatus.equals(Orders.PAID)) {
            String refund = weChatPayUtil.refund(ordersDB.getNumber(),//订单号
                    ordersDB.getNumber(),//退款订单号
                    new BigDecimal(0.01),//退款金额，单位 元
                    new BigDecimal(0.01));
    ...
    public void adminCancel(OrdersCancelDTO ordersCancelDTO) throws Exception {
        ...
        Orders orders = Orders.builder().id(ordersCancelDTO.getId()).status(Orders.CANCELLED).cancelReason(ordersCancelDTO.getCancelReason()).cancelTime(LocalDateTime.now()).build();
